...
import { ExcelOptions,CellPosition } from "./type"; 
export class ExcelRender {
    ...
    constructor(canvas:HTMLCanvasElement, ctx:CanvasRenderingContext2D, excelData:ExcelData, options:ExcelOptions) {
        this.canvas = canvas;
        this.ctx = ctx;
    ...
    setScrollOffset(offsetX: number, offsetY: number) {
        this.scrollOffsetX = offsetX;
        this.scrollOffsetY = offsetY;
    }

    setEditingCell(row: number, col: number) {
    ...
    // 绘制尺寸调整辅助线
    drawResizeGuideLine(newSize: number, resizeType:'row'|'col' | null, resizeIndex: number) {
        this.ctx.save();
        this.draw(); 
    ...
    getRowYPosition(rowIndex: number): number {
    ...
    getColXPosition(colIndex: number) : number{
